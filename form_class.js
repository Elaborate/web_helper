...
class form {
    constructor(formName) {
        ...
        this.loaded = false;
        this.formName = "default_form";
        this.filename = "default_form.json";
  ...
  `;
    ...
    mainHTML(id = 0) { return "This is a form!"; }
    //-----------------------------------------------------
    //-----------------------------------------------------
    ...
    saveSettingsToFile() {
        ...
        fs.writeFile(this.filename, JSON.stringify(this.parameters, null, 2), function (err) {
            if (err)
                return console.log("Couldn't save data from form" + this.formName + "Error: " + err);
    ...
    display() {
        ...
        if (form.window === undefined) {
            ...
            if (this.targetForm)
                this.targetForm.innerHTML = html;
        }
        else
            try {
        ...
        `);
                ...
                console.log("Cannot find BrowserWindow or document!");
                return;
            }
    }
    //-----------------------------------------------------
    ...
    static parametersToObject(formData) {
    ...
    static message(name, formData) {
        ...
        if (!formObj || !formObj.formName)
            console.log("cannot find form " + formName);
        const method = formObj[order];
        if (!method)
            console.log("cannot find method " + order);
        try {
            formObj[order](args);
            //method(args)
        }
...
//--------------------------------------------------------------------------------------------
module.exports = form;
//# sourceMappingURL=form_class.js.map