...
export class website {
  static test1(): string {
    return "inheritance works.<br/>\n";
  }
  static test2(): string {
    return "overriding doesn't work.<br/>\n";
  }
  static test3(): string {
    return "This is from the original Class...<br/>\n";
  }
  static test4(): string {
    return this.test3();
  }
  ...
  static update(): void {
    //How much of this is even needed?
    if (this.window) {
  ...
  static init(): void {
    ...
    this.window.webContents.on("did-finish-load", () => {
      this.window.webContents.session.cookies.get(
        {},
        async (error, cookies) => {
          if (error) {
  ...
  static changeAccountDetails(args = this): string {
    return this._changeAccountDetails(
      args.service,
      args.username,
      args.password
    );
  ...
  static _changeAccountDetails(service, username, password): string {
    console.log(
      `${
        this.siteName
      }._changeAccountDetails(${service}, ${username}, ${password})`
    );
    accounts[service] = {};
    accounts[service].username = username;
    accounts[service].password = password;
    var fs = require("fs");
    fs.writeFile("accounts.json", JSON.stringify(accounts, null, 2), function(
      err
    ) {
...
export class RRL extends website {
  static test(): string {
    return (
      this.test1() + this.test2() + this.test3() + super.test3() + this.test4()
    );
  }
  static test2(): string {
    return "overwriting works.<br/>\n";
  }
  static test3(): string {
    return "This is from the extended Class...<br/>\n";
  }
  static siteName = "RRL";
  static service = "RRL";
  static fictions: any;
  ...
  static async Login(
    ...
    console.log(
      `Logging in to '${uri}' as '${username}', with pw: '${password}'.`
    );
