...
export class form {
  ...
  compose(id = 0): string {
    ...
    return `
      <form id=${this.formName} onSubmit="JavaScript:sendForm(event, '${
      this.formName
    }')"> 
        <fieldset>
  ...
  */
  ...
  saveSettingsToFile(): void {
    ...
    fs.writeFile(
      ...
      function(err) {
        if (err)
          return console.log(
            "Couldn't save data from form" + this.formName + "Error: " + err
          );
  ...
  loadSettingsFromFile(): void {
    ...
    try {
      ...
      console.log(
        "successfully loaded parameter file: '" +
          file +
          "' into " +
          this.formName
      );
  ...
  static message(name, formData) {
    ...
    try {
      ...
      console.log(
        `${formName} method '${args["order"]}' did not work. ${e.message}`
      );
    }
  }
